...
// Hóa đơn cuối cùng vừa tạo (dùng cho in)
...
function renderStatistics(){
    // Tổng sản phẩm (số loại)
    let productCount = Object.keys(products).length;
    // Số nhân viên
    let staffCount = staffs.length;
    // Doanh thu
    let totalRevenue = Number(localStorage.getItem('revenue')) || revenue || 0;
    // Số sản phẩm đã bán (tổng số lượng trong invoices)
    let soldCount = 0;
    invoices.forEach(function(inv){
        inv.items.forEach(function(it){ soldCount += Number(it.qty); });
    });

    if(document.getElementById('tkProduct')) tkProduct.innerHTML = productCount;
    if(document.getElementById('tkStaff')) tkStaff.innerHTML = staffCount;
    if(document.getElementById('tkMoney')) tkMoney.innerHTML = totalRevenue.toLocaleString() + ' VNĐ';
    if(document.getElementById('tkSold')) tkSold.innerHTML = soldCount;

    // Lịch sử hoạt động: show recent invoices (time, staff, activity)
    ...
    invoices.slice().reverse().slice(0,20).forEach(function(inv){
        let tr = document.createElement('tr');
        let tdTime = document.createElement('td');
        tdTime.textContent = inv.date;
        let tdStaff = document.createElement('td');
        tdStaff.textContent = inv.staff || '';
        let tdAct = document.createElement('td');
        tdAct.textContent = `Thanh toán ${inv.id} - ${inv.total.toLocaleString()} VNĐ`;
...
function renderInvoices(filteredInvoices) {
    ...
    let list = Array.isArray(filteredInvoices) ? filteredInvoices : invoices;
    ...
    list.slice().reverse().forEach(function (invoice) {

        let productNames = invoice.items.map(function (item) {
        ...
        tbody.innerHTML += `
            ...
            <tr>
                ...
                <td>${invoice.status || 'Đã thanh toán'}</td>
...
function searchInvoice() {
    let query = document.getElementById('searchInvoiceCode').value.trim().toUpperCase();
    if (query === '') {
        renderInvoices();
        return;
    }
    query = query.replace(/\s+/g, '');
    let results = invoices.filter(function (invoice) {
        let invoiceId = (invoice.id || '').toUpperCase();
        return invoiceId.includes(query) || invoiceId.endsWith(query);
    });
...
function showQrModal(total, invoiceCode, items) {

    let qrBox = document.getElementById("qrCodeBox");

    qrBox.innerHTML = `

        <img src="QR.jpg" alt="QR thanh toán" style="border:1px solid #ddd;border-radius:12px;padding:8px;background:#fff;">
    ...
    let itemList = items.map(function (item) {
        ...
        return `<li>${item.name} x${item.qty} - ${(item.price * item.qty).toLocaleString()} VNĐ</li>`;
...
function pay() {
    ...
    let invoiceCode = "HD" + Date.now().toString().slice(-8);

    let invoiceItems = cart.map(function (item) {
    ...
    let status = document.getElementById('invoiceStatus') ? document.getElementById('invoiceStatus').value : 'Đã thanh toán';
    ...
    invoices.push({
        ...
        staff: currentStaff ? currentStaff.name + " | " + currentStaff.id : "",
    ...
    showQrModal(total, invoiceCode, invoiceItems);

    // Lưu hóa đơn mới nhất để có thể in
    lastInvoice = {
        id: invoiceCode,
        date: new Date().toLocaleString("vi-VN"),
        total: total,
    ...
    // Hiển thị nút in (nếu tồn tại)
    try{
        let btn = document.getElementById('printInvoiceBtn');
...
// In hóa đơn: mở cửa sổ in với nội dung hóa đơn
function printInvoice(invoice) {
    if (!invoice) return;
    let itemsHtml = invoice.items.map(function(i){
        return `<tr><td>${i.name}</td><td>${i.qty}</td><td>${i.price.toLocaleString()} VNĐ</td><td>${(i.price*i.qty).toLocaleString()} VNĐ</td></tr>`;
    }).join("");

    let html = `<!doctype html><html><head><meta charset="utf-8"><title>Hóa đơn ${invoice.id}</title><style>body{font-family:Arial,Helvetica,sans-serif;padding:20px}h1{margin-bottom:8px}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#f4f4f4}</style></head><body>`
        + `<h1>SIÊU THỊ HÀ PHƯƠNG</h1>`
        + `<p><strong>Mã hóa đơn:</strong> ${invoice.id}</p>`
        + `<p><strong>Ngày:</strong> ${invoice.date}</p>`
        + `<p><strong>Nhân viên:</strong> ${invoice.staff}</p>`
        + `<table><thead><tr><th>Tên</th><th>Số lượng</th><th>Đơn giá</th><th>Thành tiền</th></tr></thead><tbody>`
        + itemsHtml
        + `</tbody></table>`
        + `<h2 style="margin-top:12px">Tổng: ${invoice.total.toLocaleString()} VNĐ</h2>`
        + `</body></html>`;

    let w = window.open('', '_blank', 'width=700,height=800');
